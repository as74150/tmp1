...
class Switches(app_manager.RyuApp):
    ...
    def state_change_handler(self, ev):
	    ...
	    print('register %s', switch.__str__())
    ...
    def lldp_packet_in_handler(self, ev):
		...
		if src_dpid == int(links.split(',')[0]) and dst_dpid == int(links.split(',')[1]):
		    self.sendCfgMessage()
	    tmpStr = str(src_dpid)+','+str(dst_dpid)+','+str(src_port_no)+','+str(dst_port_no)+','+str(0)+'#' 
    ...
    def sendCfgMessage(self):
	strTopoMessage = ''
	for lk in self.interLinks:
	    strTopoMessage = strTopoMessage + lk
	strTopoMessage = strTopoMessage.rstrip('#')
	try:
	    f_rf_switch=open("interLinks.log","w+")
